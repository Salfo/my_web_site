...
# The idea of dimension reduction from matrix factorization prospective
...
The trick is to guess some initial values for $Z$, and use it to estimate $B$, then use the estimated $B$ to estimate a new $Z$. Use the new $Z$ to estimate a new $B$. Continue the iteration untill some stopping criterion. Thus, we estimate $Z$ and $B$ iteratively. When do we stop the iteration?
...
Thus, we minimize $Q$, the objective function. Following is an example implementation of a simple least squares algorithm.
...
Observe that row 1 of $B$ has high values in columns 1 and 2 compared to columns 3, 4, and 5; and row 2 has higher values for columns 4 and 5 compared to columns 1, 2, and 3. It is reasonable to infer that `Topic.1` is about education, and `Topic.2` is about health. 
...
Observe that the source of any numerical difference between $b_{1,1}$ and $b_{1,2}$ is the numerical difference between the first and second column of $W$. Also, observe that, whatever $F$ is, $b_{1,1}$ is a total weight of the first variable $W_1$ (say the first word). Likewise, $b_{1,2}$ is a total weight of the second variable $W_2$ (say the second word); and so on untill $b_{1,p}$. Put differently, $b_{1,j}$ is a total weight of the word $W_j$. Thus, the coefficients $[b_{1,1},b_{1,2}, \cdots,b_{1,p}]$ are the weight of the words $W_1, W_2, \cdots, W_p$, respectively. If these are words' weights, it is natural to use the words with highest weights to name row 1 of $B$. We name the remaining rows of $B$ in similar fashion.
...
The numerical difference between $z_{1,1}$ and $z_{1,2}$ stems from the numerical difference between the weights in column $1$ and $2$ of the weights matrix $N$.  The numerical difference between $z_{1,1}$ and $z_{2,1}$ stems from the numerical difference between the words counts in documents $1$ and $2$ of the words counts matrix $W$.
...
I hope this introductory exposition of topic modeling provides an intuitive understanding of the why, and how of the subject. Feel free to leave your comments.